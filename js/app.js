(() => {
  ...
  const teamName=id=>window.SmartPortTeam?.categoryName(S.teamConfig,id)||id||'—';
  const teamColor=id=>window.SmartPortTeam?.categoryColor(S.teamConfig,id)||'#667085';
  const teamOptions=(selected='')=>(S.teamConfig?.categories||[]).filter(item=>item.active!==false||item.id===selected).map(item=>({value:item.id,label:`${item.name} (${item.id})${item.active===false?' · 停用':''}`}));
  const assignedMember=categoryId=>window.SmartPortTeam?.responsibleMember(S.teamConfig,categoryId)?.name||'未設定負責人';
  ...
  function renderGantt(){
    ...
    buildGanttRows().forEach(t=>{const row=document.createElement('div');row.className='g-row'+(t._kind==='sub'?' sub-row-gantt':'');const owner=t.owner||'',r=riskOf(t),p=progressOf(t);row.innerHTML=`<div class="g-cell"><span class="owner-pill">${esc(teamName(owner))}</span></div><div class="g-cell"><b>${esc(t.id)}</b> ${esc(t.name||'')}<div><span class="risk-pill risk-${r}">${esc(r.replace('_',' '))}</span> <span class="muted">${p==null?'—':p+'%'}</span></div></div><div class="g-cell timeline"></div>`;const tl=row.querySelector('.timeline');makeCpLines(tl,today);const bar=document.createElement('div');bar.className='bar';bar.style.left=pos(d(t.start))+'%';bar.style.width=Math.max(.5,pos(d(t.end))-pos(d(t.start)))+'%';bar.style.setProperty('--bar-color',teamColor(owner));bar.title=`${t.id} · ${t.name}\n${t.start} → ${t.end}\nProgress: ${p==null?'Not Updated':p+'%'}`;bar.onclick=()=>t._kind==='wp'?openWpDetail(t.id):openSubtaskDetail(t.id);bar.innerHTML=`<div class="fill" style="width:${p==null?0:p}%"></div><div class="bar-label">${esc(t.id)} · ${esc(t.name||'')}${p==null?'':' · '+p+'%'}</div>`;tl.appendChild(bar);root.appendChild(row)})
  }
  ...
  function renderPlan(){
    ...
    S.workPackages.forEach(w=>{
      ...
      tb.insertAdjacentHTML('beforeend',`<tr class="plan-row clickable" data-open-wp="${esc(w.id)}"><td><b>${esc(w.id)}</b> ${esc(w.name)}<div class="muted">${esc(w.group||'')}</div></td><td>${esc(teamName(w.owner))}<div class="muted">${esc(assignedMember(w.owner))}</div></td><td>${esc(w.start)}</td><td>${esc(w.end)}</td><td>${esc(weightOf(w))}</td><td>${p==null?'—':p+'%'}<div class="muted">${esc(st)}</div></td><td>${tags(w.ifs)}<br>${tags(w.fsrs)}</td><td><div class="row-actions"><button class="btn smallbtn" data-edit-wp="${esc(w.id)}">Edit</button></div></td></tr>`);
      S.subtasks.filter(s=>s.parent_wp===w.id).forEach(s=>{const sp=progressOf(s);tb.insertAdjacentHTML('beforeend',`<tr class="plan-row subtask-plan-row clickable" data-open-sub="${esc(s.id)}"><td style="padding-left:28px">↳ <b>${esc(s.id)}</b> ${esc(s.name)}<div class="muted">Parent: ${esc(s.parent_wp||'')}</div></td><td>${esc(teamName(s.owner_team))}<div class="muted">${esc(assignedMember(s.owner_team))}</div></td><td>${esc(s.start||'')}</td><td>${esc(s.end||'')}</td><td>${esc(weightOf(s))}</td><td>${sp==null?'—':sp+'%'}<div class="muted">${esc(statusOf(s))}</div></td><td>${tags(s.ifs)}<br>${tags(s.fsrs)}</td><td><div class="row-actions"><button class="btn smallbtn" data-edit-subtask="${esc(s.id)}">Edit</button></div></td></tr>`)})
    });
  ...
  function selectField(name,label,value,options){return`<div class="field"><label>${esc(label)}</label><select name="${esc(name)}">${options.map(option=>{const x=typeof option==='object'?option:{value:option,label:option};return`<option value="${esc(x.value)}" ${String(value)===String(x.value)?'selected':''}>${esc(x.label)}</option>`}).join('')}</select></div>`}
  ...
  function editSubtask(id=''){
    ...
    if(!old)return;
    bindForm(id?`編輯 ${id}`:'新增 Subtask',`${field('id','Subtask ID',old.id,'text',!!id)}${field('parent_wp','Parent WP',old.parent_wp)}${field('name','Name',old.name)}${selectField('owner_team','Owner Team',old.owner_team||'CTL',teamOptions(old.owner_team||'CTL'))}<div class="split">${field('start','Start',old.start||'','date')}${field('end','End',old.end||'','date')}</div><div class="split">${field('weight','Weight',weightOf(old),'number')}${field('target_cp','Target CP',old.target_cp||'')}</div>${field('ifs','IF（逗號分隔）',csv(old.ifs))}${field('fsrs','FSR（逗號分隔）',csv(old.fsrs))}${field('keywords','Keywords（逗號分隔）',csv(old.keywords))}${area('description','工作內容',old.description||'')}${area('expected_evidence','Expected Evidence（每行一項）',arr(old.expected_evidence).join('\n'))}${formButtons(id?`<button type="button" class="btn danger" data-delete-subtask="${esc(id)}">Archive</button>`:'')}`,async fd=>{
  ...
  function exportSnapshot(){const blob=new Blob([JSON.stringify({project:S.project,work_packages:S.workPackages,subtasks:S.subtasks,functional_safety_requirements:S.fsrs,checkpoints:S.checkpoints,team_config:S.teamConfig},null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`SmartPort_Project_Snapshot_${new Date().toISOString().slice(0,10)}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),500)}
  function renderAll(){renderSummary();renderGantt();renderPlan();renderFsr();renderCp()}
  async function load(){
    try{
      const me=await API.me();
      window.SMARTPORT_ACCESS={...(window.SMARTPORT_ACCESS||{}),...me};
      $('#githubUser').value=me.login;
      const headerLogout=$('#btnHeaderLogout');
      if(headerLogout)headerLogout.hidden=!me.authenticated;
      if(['UNAUTHENTICATED','DENIED'].includes(me.role)){
        setConnection(false,me.role==='DENIED'?'帳號未授權':'等待登入');
        return;
      }
      const snap=await API.loadSnapshot();
      window.SmartPortStore.replaceSnapshot(snap);
      const connected=API.getMode?.()==='supabase'?`Supabase Gateway · ${me.login}`:`本機 Project Store 已連線 · ${me.login}`;
      setConnection(true,me.public_snapshot?`Public Snapshot · Read Only · ${String(snap.generated_at||'').slice(0,10)||'unknown date'}`:connected);
      renderAll();
    }catch(e){
  ...
  document.addEventListener('smartport:team-config-saved',renderAll);
