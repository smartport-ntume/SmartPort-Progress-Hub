(() => {
  const groups=[
    ...
    {id:'requirements',label:'Requirements',views:['fsr','reference','tr']},
  ...
  function install(){
    ...
    function sync(){
      groups.forEach(g=>{
        const wrap=nav.querySelector(`[data-nav-group="${g.id}"]`);if(!wrap)return;
        const active=g.views.some(v=>buttons[v]?.classList.contains('active'));
        wrap.classList.toggle('group-active',active);
      });
    }
    ...
    const style=document.createElement('style');style.textContent=`
      ...
      .nav-menu-trigger{display:flex!important;align-items:center;gap:7px;white-space:nowrap}
      .nav-menu.group-active>.nav-menu-trigger{background:#fff;color:var(--navy)}
      .nav-caret{font-size:10px;opacity:.75}
      .nav-menu-popover{display:none;position:absolute;right:0;top:calc(100% + 8px);min-width:230px;background:#fff;border:1px solid var(--line);border-radius:11px;padding:7px;box-shadow:0 12px 30px rgba(16,24,40,.18);z-index:120}
      .nav-menu.open>.nav-menu-popover{display:flex;flex-direction:column;gap:3px}
      .nav .nav-menu-item{display:block!important;width:100%;text-align:left;background:#fff;color:var(--text);border-radius:7px;padding:9px 11px}
      .nav .nav-menu-item:hover{background:#f4f7fb}
      .nav .nav-menu-item.active{background:#eef4fb;color:var(--navy);font-weight:700}
      .nav-system{margin-left:2px}
      .nav-system-button{white-space:nowrap}
      @media(max-width:1100px){.header-right{width:100%}.nav.nav-grouped{flex-wrap:wrap;justify-content:flex-start}.nav-menu-popover{left:0;right:auto}}
    `;document.head.appendChild(style);
  }

  const obs=new MutationObserver(()=>install());
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{install();obs.observe(document.body,{childList:true,subtree:true})});
