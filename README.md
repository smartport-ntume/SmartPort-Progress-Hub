...
若既有週報顯示 `Not assessed`、`Unable to assess ... without file access` 或無法讀取週報／專案資料，卻出現三項 0 分，請更新並重啟 Agent、重新整理網站，選擇該成員後按 **重新批改原始週報**。即使錯誤只出現在「需要補充」或批改注意事項，也會在讀取明細時顯示「批改未完成」、隱藏無效分數並禁止核准。已歸檔的 Word 可直接重跑，不需重新上傳、不需新增 SQL；真正讀完但內容不足的週報仍可得到 0 分。

進度提案也支援「工作紀錄更新（百分比不變）」：有具體成果、狀態或障礙，但沒有可信的整體完成百分比時，仍可讓 PM 勾選採用；核准後保留原本百分比，未知值也不會變成 0。明確自報的任務完成度可提出待 PM 確認的百分比，並另外顯示自報值、缺少的驗證與注意事項；就緒程度及單一步驟完成度不會換算成整體完成度。新紀錄會保留既有證據，並標示週報來源及待確認事項。若舊批改只因 `actual_progress` 為空或未完成驗收而產生 0 項，更新 Agent 後重新批改即可使用新規則；不會憑空為空白範本建立提案。
...
**下一批須等上一批全員完成 PM 審閱才建立及發送。** 以每人最新提交版本為準；已核准或已附回饋退回補件都算 PM 已審閱，未繳、批改中、批改失敗、待審或審核失敗都會暫停發送。成員再補交新版後，需要重新完成該版審閱。管理中心列出等待中的成員；PM 審完時 Agent 會重新檢查排程，離線恢復或等待期間也會每 15 分鐘重試。首批沒有前期週報時可正常建立。跨過一週仍未審完，不預建多批過期週報；恢復時使用當時的週次和截止日。

發送前會讀取最新正式進度與上期 PM 意見，各人的 Word 及入口重新下載的文件均加入 **上期 PM 回饋與本週回覆**，保留原意見、來源週次、提交版本與審閱結果，並提供本週處理結果和待辦回覆欄。上期 PM 文字本身不算本週完成證據。**補發 Discord** 也會檢查上一批審閱；發送後的附件不會自動被改寫。已送出的部分附件會沿用同一份資料；若上一期回饋在續送前改變，需由 PM 確認後補發整批。

當 PM 修改既有成員姓名時，Agent 會把新名字同步到仍在收件期限內的週報批次與催繳名單，但不改動當週凍結的分類、任務與 CP 範圍。已發到 Discord 的舊附件不會被重貼；從週報入口重新下載即可取得新姓名版本。

預設截止時間是次週一 12:00；截止後仍可直接補交，系統依提交時間標記逾期，不需 PM 展延。每則 Discord 訊息最多附五份 Word，人數較多時會自動分批；Agent 會記錄已送出的批次，重啟後從未完成的下一批續送。若 Agent 在發布時間關機，只要在下一個發布週期前重新啟動就會補發。可選的每日催繳只列出尚未成功上傳的姓名與上傳網址，不會重複附檔或自動 mention Discord 帳號。
...
- 勾選要採用的進度更新，核對目前值與提案值後，按 **核准勾選項目並結案**。未勾選項目記為未採用。有效批改若沒有產生可採用提案，會顯示缺漏及批改注意事項，並提供 **核准週報（不更新進度）**；只有實際產生的提案才會出現勾選框。**退回補件** 必須填寫原因，不寫入正式進度；退回後需重新批改或補交新版才能再次審核。
- **重新批改原始週報** 會使用已歸檔的 Word，不依賴已刪除的 Storage 暫存；需要 `can_trigger_codex`。已核准版本不能重新批改，需補交新版。
- 排入批改後可立即切換其他成員繼續操作；只鎖住正在處理的週報，重新整理分頁會恢復追蹤各筆已排入的工作。另一位成員處理完成時，不會清除目前正在填寫的 PM 回饋或勾選項目。
- 舊的無效批改若已卡在「審核未完成」且尚未寫入進度，可先按 **解除失敗審核**，再重新批改；已部分核准的紀錄仍保留原有保護。
- **補發 Discord** 會重送當週附件與原連結；**展延截止** 以台灣時間調整截止日；截止時間用來判定逾期，不會停止補交。展延不自動發送訊息，需通知時再按補發。
...
- 管理中心的「需要補充／建議下一步」自動依工作 ID 或唯一完整工作名稱歸入 WP／Subtask，不需要 PM 選擇工作。舊版混在整份週報的建議也會自動拆分；同一 WP 多個子任務的共通建議只列在該 WP，日期或格式等共通事項留在 Word 前段。沒有明確對應的文字保留為共通事項，不猜測工作。按「儲存回饋」可先保存草稿；核准及退回時也會保存。原始 AI 分析另外保留。
- 回饋欄使用 16px 字體、1.75 倍行距及至少約八行的高度；支援 `field-sizing` 的瀏覽器會隨內容展開，其他瀏覽器仍可拖曳加高。新批改、管理中心、成員入口與 Word 共用相同工作對應規則。
- 下期 Word 將工作回饋放在對應任務填寫區，提供處理結果與佐證回覆欄。WP 共通建議只列一次；已完成但仍有回饋的工作保留追蹤欄。整體 PM 意見與共通回饋仍在 Word 前段。請在最後核准前確認意見；已經送出的附件不會隨後續編輯改寫。
- 成員自報 30% 等明確完成度會列入提案；即使不是原範本必填項目，只要屬於該成員且有本週具體內容，也可提案。PM 可在核准前調整 0～100%（含往下修正）；留白保留原進度。自報值、原提案、核定值分別保留，若未達 100%，不沿用提案的 Completed 狀態。
- 進度修正、回饋和選取項目一併存入審核工作；失敗重試沿用原核定值，避免同一批部分成功、部分使用另一版本。原有最新提交版本與正式進度衝突檢查仍適用。
- 截止後及舊七天補交期過後均可上傳，入口與 Storage RLS 使用相同規則；仍保留上傳憑證的 15 分鐘有效期、檔案限制，以及明確關閉／取消批次的限制。審核正在寫入時，暫時阻擋同一份新版上傳。

**既有系統升級順序：**

1. 先停止 Windows Agent；已安裝週報審核版的系統，在 Supabase SQL Editor 執行 `supabase/migrations/202609250001_weekly_editable_feedback.sql`。全新安裝需依序執行先前各 migration（含 `202609110002_weekly_review_cycle.sql`）。
...
已完成上述 SQL 的系統，2026-09-26 的自動工作對應與閱讀版面更新不需再執行 SQL；更新並重啟 Agent 後，網站按 Ctrl+F5。排程啟動者由工作排程器停止及重啟原工作。
