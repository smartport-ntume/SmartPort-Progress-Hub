...
當 PM 修改既有成員姓名或 Discord 使用者 ID 時，Agent 會把設定同步到仍在收件期限內的週報批次與催繳名單，但不改動當週凍結的分類、任務與 CP 範圍。已停用、移除或取消「需交週報」的成員會停止催繳，既有週報仍保留。已發到 Discord 的舊附件不會被重貼；從週報入口重新下載即可取得新姓名版本。

預設截止時間是次週一 12:00，並保留七天補交期。每則 Discord 訊息最多附五份 Word，並附上對應成員的個人繳交連結；人數較多時自動分批。Agent 會記錄已送出的附件批次，重啟後從未完成的下一批續送。若 Agent 在發布時間關機，只要在下一個發布週期前重新啟動就會補發。選用的每日催繳依最新提交版本，只提醒未繳、上傳／批改失敗或 PM 退回補件的人；已上傳排隊中、批改中、待 PM 審核及已核准者均不催繳。
...
PM 可在成員設定填入選用的 **Discord 使用者 ID** 並儲存。請依 [Discord 官方取號說明](https://support.discord.com/hc/en-us/articles/206346498-Where-can-I-find-my-User-Server-Message-ID)，開啟「使用者設定 → 進階 → 開發者模式」，對成員按右鍵選「複製使用者 ID」。欄位接受完整數字 ID，不接受顯示名稱或 `@名稱`；同一個 ID 不可對應多位成員。設定保存在 Private Git，Guest 快照不包含成員名單或 Discord ID。

將 Agent 的 `WEEKLY_REMINDER_ENABLED` 改為 `true` 並重啟後，每日催繳才會啟用；預設仍為 `false`。已填 ID 的待繳成員會收到精準 `@` 標記，未填者仍列姓名與連結。每則提醒只允許名單中的使用者 mention，不允許 `@everyone`、角色或姓名中的文字觸發額外通知；過長名單會分則送出。全部送出並成功記錄後，同一批次同一天不再重送；若部分訊息成功後斷線或記錄失敗，重試可能重複已送出的部分。

每週發布訊息會列出每位成員的個人連結；PM 也可在週報管理中心選擇成員，按 **複製個人繳交連結**。連結只會預選姓名，仍沿用共用批次 token，並可切換成員；Discord ID 僅供通知對應，沒有加入 Discord 登入或身分驗證。

已完成下方「PM 週報管理中心」升級的系統，**本次不需新增 SQL 或套件**。請先停止 Agent，取得此版程式並完成檢查、再重啟，最後重新整理網站並填寫 ID；舊版 Agent 尚不會保留這個欄位。既有 Discord 訊息不會因更新而自動重送。
...
成員沿用 Discord 的免密碼入口，選擇姓名即可看到分數、缺漏、建議、PM 回饋及歷史版本。批改完成、PM 審核中、退回補件、已核准是不同狀態。入口仍是原本的共用批次憑證：持有連結者可以切換成員查看回饋，請維持受控頻道使用；Discord 通知對應不改變這個存取方式。
