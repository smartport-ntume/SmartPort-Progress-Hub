-- Scheduled Discord weekly-report batches and a guest-password submission portal.
-- The Discord webhook stays on the Windows Agent. Private Git remains source of truth.

begin;

create table if not exists public.weekly_report_batches (
  ...
  pm_user_id uuid not null references public.profiles(user_id) on delete restrict,
  status text not null default 'OPEN' check (status in ('OPEN', 'CLOSED', 'CANCELLED')),
  created_by_agent text not null default '',
  discord_message_id text not null default '',
  discord_message_sent_at timestamptz,
  last_reminder_date date,
  last_error text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  check (accept_until >= due_at),
  check (octet_length(payload::text) <= 4194304),
  check (last_error is null or char_length(last_error) <= 2000)
);

create table if not exists public.weekly_report_upload_grants (
  id uuid primary key default gen_random_uuid(),
  batch_id uuid not null references public.weekly_report_batches(id) on delete cascade,
  member_id text not null check (member_id ~ '^[A-Za-z0-9][A-Za-z0-9_.-]{0,99}$'),
...
create table if not exists public.weekly_report_submissions (
  id uuid primary key default gen_random_uuid(),
  batch_id uuid not null references public.weekly_report_batches(id) on delete cascade,
  upload_grant_id uuid not null unique references public.weekly_report_upload_grants(id) on delete restrict,
  member_id text not null,
  member_name text not null,
  submitted_by uuid not null references auth.users(id) on delete restrict,
  storage_path text not null unique,
  filename text not null,
  job_id uuid unique references public.gateway_jobs(id) on delete set null,
...
create index if not exists weekly_report_batches_status_idx
  on public.weekly_report_batches (status, report_date desc);
create index if not exists weekly_report_upload_grants_expiry_idx
  on public.weekly_report_upload_grants (expires_at) where used_at is null;
create index if not exists weekly_report_submissions_batch_idx
  on public.weekly_report_submissions (batch_id, submitted_at desc);

drop trigger if exists weekly_report_batches_touch_updated_at on public.weekly_report_batches;
create trigger weekly_report_batches_touch_updated_at
before update on public.weekly_report_batches
for each row execute function public.smartport_touch_updated_at();

drop trigger if exists weekly_report_submissions_touch_updated_at on public.weekly_report_submissions;
create trigger weekly_report_submissions_touch_updated_at
before update on public.weekly_report_submissions
for each row execute function public.smartport_touch_updated_at();

alter table public.weekly_report_batches enable row level security;
alter table public.weekly_report_upload_grants enable row level security;
alter table public.weekly_report_submissions enable row level security;

revoke all on public.weekly_report_batches from anon, authenticated;
revoke all on public.weekly_report_upload_grants from anon, authenticated;
revoke all on public.weekly_report_submissions from anon, authenticated;

grant select on public.weekly_report_submissions to authenticated;
grant select, insert, update, delete
on table
  public.weekly_report_batches,
  public.weekly_report_upload_grants,
  public.weekly_report_submissions
to service_role;

drop policy if exists weekly_report_submissions_read_own_or_pm on public.weekly_report_submissions;
create policy weekly_report_submissions_read_own_or_pm on public.weekly_report_submissions
for select to authenticated
using (submitted_by = auth.uid() or public.smartport_role() = 'PM');
...
declare
  ...
  v_submissions jsonb;
begin
  if auth.uid() is null or public.smartport_role() not in ('GUEST', 'ENGINEER', 'PM') then
    raise exception 'weekly_portal_login_required' using errcode = '42501';
  end if;
...
as $$
  select coalesce(exists (
    select 1
    from public.weekly_report_upload_grants g
    join public.weekly_report_batches b on b.id = g.batch_id
    where g.storage_path = p_name
      and g.created_by = auth.uid()
      and g.used_at is null
      and g.expires_at > now()
      and b.status = 'OPEN'
      and b.accept_until >= now()
  ), false)
$$;

revoke all on function public.smartport_valid_weekly_upload_path(text) from public, anon;
grant execute on function public.smartport_valid_weekly_upload_path(text) to authenticated;

drop policy if exists smartport_weekly_portal_insert on storage.objects;
create policy smartport_weekly_portal_insert on storage.objects
for insert to authenticated
with check (
  bucket_id = 'weekly-reports'
  and public.smartport_valid_weekly_upload_path(name)
);

drop policy if exists smartport_weekly_portal_delete_unsubmitted on storage.objects;
create policy smartport_weekly_portal_delete_unsubmitted on storage.objects
...
as $$
begin
  update public.weekly_report_submissions
  set
    status = new.status,
...
drop trigger if exists gateway_jobs_sync_weekly_submission on public.gateway_jobs;
create trigger gateway_jobs_sync_weekly_submission
after update of status, error on public.gateway_jobs
for each row execute function public.smartport_sync_weekly_submission();

alter table public.weekly_report_submissions replica identity full;
...
begin
  if not exists (
    ...
    where pubname = 'supabase_realtime'
      ...
      and tablename = 'weekly_report_submissions'
  ) then
    alter publication supabase_realtime add table public.weekly_report_submissions;
  end if;
end;
$$;
